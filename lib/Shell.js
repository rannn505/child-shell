...
class Shell extends EventEmitter {
  constructor(options = {}) {
   ...
   https://rannn505.gitbook.io/node-powershell/shell/initialize`);
    ...
    this.psOut = new ShellStreamBuffer();

    this.streams.stdout.pipe(this.psOut);
    this.streams.stderr.pipe(this.psOut);
  }
  ...
  invoke() {
    ...
    return new Promise((resolve, reject) => {
      psOut.once('EOI', () => {
        ...
        this.streams.stderr.removeAllListeners('data');
  ...
  dispose() {
    this.streams.stdout.unpipe(this.psOut);
    this.streams.stderr.unpipe(this.psOut);
    this.psOut.end();
