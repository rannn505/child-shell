...
class Shell extends EventEmitter {
  constructor(options = {}) {
    ...
    //     return new Promise(() => {});
    //   }
    //   return this[fn].call(this, proc);
    // }

    this.invoke = () => new Promise((resolve, reject) => setImmediate(this.invoke.bind(this, proc, resolve, reject)));
    this.stop = this.stop.bind(this, proc);
    this.dispose = new Promise((resolve, reject) => setImmediate(this.dispose.bind(this, proc, resolve, reject)));
  ...
  addArgument(argument = '') {
    ...
    lastCommand.addArgument(argument);
    return this.addCommand(lastCommand);
  }
  addParameter(parameter = {}) {
    const lastCommand = this.commands.pop();
    lastCommand.addParameter(parameter);
    return this.addCommand(lastCommand);
  }
  ...
  invoke(proc, resolve, reject) {
    // return new Promise((resolve, reject) => {
      ...
      this.streams.stderr.once('data', () => { invocationHadErrors = true });
      psOut.once('finish', () => {
        this.streams.stdout.unpipe(psOut);
        this.streams.stderr.unpipe(psOut);
        
        let output = psOut.getContentsAsString();
      ...
      shellSafeWrite(this.streams.stdin, invocationCommands)
        ...
        .then(() => {
          ...
          if(this.verbose) {
            ...
            logger.info(invocationCommands);
          }
        })
  ...
  stop(proc) {
    if(this.invocationStateInfo === 'Running') {
      proc.kill(os.constants.signals.SIGINT);
      this.invocationStateInfo = 'Stopping';
    }
    return Promise.resolve();
  }
  dispose(proc, resolve, reject) {
